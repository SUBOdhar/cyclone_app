...
class FileService {
  ...
  Future<void> handleDownload(String filename, BuildContext context) async {
    ...
    _performDownloadWebWithProgress(url, filename, context);
  }
...
// Function to perform the download with progress (WEB)
  Future<void> _performDownloadWebWithProgress(
    String url,
    ...
    try {
      // Use Dio for the web download as well to get progress.
      final response = await dio.get(
        ...
        options: Options(
          responseType: ResponseType.stream, //  Changed to ResponseType.stream
        ),
      );

      // After successful download, trigger the actual file download using the html anchor element.
      if (response.data != null) {
        final bytes = await response.data.stream
            .toBytes(); // Get the bytes from the stream
        final blob =
            html.Blob([bytes], 'application/octet-stream'); //  MIME type.
  ...
  void _handleNetworkError(dynamic error) {
    if (error is http.ClientException) {
      print('HTTP Client Exception: ${error.message}');
    } else {
